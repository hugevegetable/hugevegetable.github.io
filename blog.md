...
>​	自己建的博客真好看，可惜没时间打理，而且只能自己用，。当个存储桶。
>
>
...
> ​	找了好久，一直不知道头文件哪里引入的js、css，最后在啥head.pug里引入的库，最后还是gpt告诉我的模板文件找到的，太难了。在这些模板文件改来改去，我也是服了。
>
> ​	改了好久，发现最开始是正确的（符合我的想法），刷新才变，在里面跳转不变。改成顺序，理清执行的顺序，这些变量，或没加条件的是先运行的。那些监听器后运行。
...
3. 本地修改/新增了一堆文件，已经git add到暂存区，想放弃修改。2
    单个文件/文件夹：
